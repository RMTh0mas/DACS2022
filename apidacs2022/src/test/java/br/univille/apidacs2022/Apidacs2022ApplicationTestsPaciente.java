...
class Apidacs2022ApplicationTestsPaciente {
	...
	@Test
    void pacienteControllerAPIPOSTDELTest() throws Exception {
      String jwtToken = getToken();

      MvcResult result = mockMvc.perform(post("/api/v1/pacientes")
				.content("{\"nome\":\"Paulo\",\"sexo\":\"Masculino\"}")
				.header("Authorization", "Bearer " + jwtToken)
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isCreated()).andReturn();
    ...
    void medicoControllerAPIPOSTPUTTest() throws Exception {
      ...
      MvcResult result = mockMvc.perform(post("/api/v1/pacientes")
				.content("{\"nome\":\"Renan\",\"sexo\":\"Masculino\"}")
				.header("Authorization", "Bearer " + jwtToken)
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isCreated()).andReturn();
      ...
      .content("{\"nome\":\"Renan2\",\"sexo\":\"Masculino\"}")
      .header("Authorization", "Bearer " + jwtToken)
      .contentType(MediaType.APPLICATION_JSON))
      .andExpect(status().isOk());
    }
